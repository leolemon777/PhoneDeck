...
当前源码版本是 **PhoneDeck 1.6.0-dev.2**，上一版完整实机稳定基线是
...
- 1.6.0-dev.2 支持通过 USB 自动交换 Wi-Fi 配对资料，随后使用证书固定的 HTTPS 在同一
...
- 多配置、宏和更完整的 Codex / Claude Code / Cursor / ZCode 工具专属指令集。
- 三台真实电脑同时在线的完整验收和自动网络发现；当前版本已完成 Windows Wi-Fi 通道与
  USB 自动配对，但仍需逐台安装接收端并完成三机实测。
...
当前首次信任建立仍使用 USB，日常操作可完全无线。电脑 IP 变化且无法自动连接时，
重新连接一次 USB 会刷新地址；后续版本将加入 mDNS 自动发现。
