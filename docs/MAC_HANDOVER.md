...
本项目当前可验证的重点为 Windows x64 与 Android。macOS 接收端为预览，iOS 尚无客户端工程。Mac 上开发不会自动补齐 Windows 设备、音频驱动或手机的实机验收。
...
需要继续完成：可信构建来源和正式签名事务接入、现有两台 Windows 与安卓的逐设备更新/回退/尾音验收、Mac 真机验收。项目中的长远四端规划保持有效，未实现项不得标记已支持。
